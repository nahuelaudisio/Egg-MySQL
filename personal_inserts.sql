...
INSERT INTO departamentos (id_depto,nombre_depto,ciudad,cod_director)
VALUES (1000,'GERENCIA','CIUDAD REAL','31.840.269');
...
/*6. Obtener los datos de los empleados cuyo cargo sea ‘Secretaria’. */
 select id_emp, nombre from empleados where cargo_emp = 'Secretaria';
...
select nombre sal_emp,comision_emp,(sal_emp+comision_emp+500)
as total_a_pagar from empleados where id_depto = 3000 order by nombre;
...
as SALARIOS_TOTAL from empleados where comision_emp > 1000000 ;  


/* 14. Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen comisión. */ 
select nombre, sal_emp,comision_emp,(sal_emp+comision_emp)
as SALARIOS_TOTAL from empleados where  comision_emp = 0;
...
nombre_depto ='VENTAS' or nombre_depto = 'INVESTIGACÍON' or nombre_depto = 'MANTENIMIENTO'; 

/* 19. Ahora obtener los nombres de los departamentos que no sean “Ventas” ni “Investigación” ni 
‘Mantenimiento. */ 
select *  from departamentos where   
nombre_depto <> 'VENTAS' and  nombre_depto <> 'INVESTIGACÍON' and  nombre_depto <> 'MANTENIMIENTO'; 
 
 /*20. Mostrar el salario más alto de la empresa.  */
select * from empleados;
select id_emp,nombre, max(sal_emp) from empleados;

/*21. Mostrar el nombre del último empleado de la lista por orden alfabético */ 
select nombre,max(id_emp) as ultimo_empleado from empleados order by nombre desc; 
...
select * from empleados;
select cargo_emp , avg(sal_emp) from empleados group by cargo_emp;
...
select nombre, count(cod_jefe) from empleados group by cod_jefe having count(cod_jefe) > 2;

/* 26. Hallar los departamentos que no tienen empleados  */
select nombre, count(cod_jefe) from empleados group by cod_jefe having count(cod_jefe) = 0;
...
select * from empleados;
select nombre, sex_emp, fec_nac from empleados
